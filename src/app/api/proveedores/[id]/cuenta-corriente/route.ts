 ...
 * facturas del proveedor (DEBE — Transmagg le debe) y pagos realizados (HABER).
 * Solo accesible para roles internos (ADMIN_TRANSMAGG, OPERADOR_TRANSMAGG).
 *
 * Ejemplos:
 * GET /api/proveedores/p1/cuenta-corriente
 * // => { proveedor, movimientos: [...], totalDebe, totalHaber, saldoFinal }
    ...
    const [facturas, pagos] = await Promise.all([
      prisma.facturaProveedor.findMany({
        where: {
          ...
          fechaCbte: { gte: desdeDate, lte: hastaDate },
        },
        select: {
          id: true,
          fechaCbte: true,
