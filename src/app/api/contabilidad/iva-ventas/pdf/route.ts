/**
 * API Route: GET /api/contabilidad/iva-ventas/pdf
 *
 * Devuelve un PDF (application/pdf) con la pestaña "IVA Ventas" tal como
 * la ve el operador en pantalla. Per CLAUDE.md regla 8 — pdfkit, inline,
 * sin HTML wrapper.
 */
...
import { generarPDFIvaVentas, type AsientoPdf } from "@/lib/pdf-libro-iva"
...
export async function GET(request: NextRequest): Promise<Response | NextResponse> {
  ...
  try {
    ...
    const pdf = await generarPDFIvaVentas(asientosPdf, periodoLabel(searchParams))
    ...
    return new Response(ab, {
      ...
      headers: {
        ...
        "Content-Disposition": `inline; filename="iva-ventas-${periodoSlug(searchParams)}.pdf"`,
