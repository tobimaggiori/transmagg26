/**
 * API Route: GET /api/contabilidad/iva-compras/pdf
 * PDF (application/pdf, inline) de la pestaña "IVA Compras". Per CLAUDE.md regla 8.
 */

import { NextRequest, NextResponse } from "next/server"
import { prisma } from "@/lib/prisma"
import { requireFinancialAccess, serverErrorResponse } from "@/lib/financial-api"
import { generarPDFIvaCompras, type AsientoPdf } from "@/lib/pdf-libro-iva"
...
export async function GET(request: NextRequest): Promise<Response | NextResponse> {
  ...
  try {
    ...
    const pdf = await generarPDFIvaCompras(asientosPdf, periodoLabel(searchParams))
    ...
    return new Response(ab, {
      ...
      headers: {
        ...
        "Content-Disposition": `inline; filename="iva-compras-${periodoSlug(searchParams)}.pdf"`,
