 ...
 * PDF (application/pdf, inline) de "Compras por Alícuota". Per CLAUDE.md regla 8.
 */

import { NextRequest, NextResponse } from "next/server"
import { prisma } from "@/lib/prisma"
import { requireFinancialAccess, serverErrorResponse } from "@/lib/financial-api"
import { generarPDFComprasPorAlicuota, type AsientoPdf } from "@/lib/pdf-libro-iva"
...
export async function GET(request: NextRequest): Promise<Response | NextResponse> {
  ...
  try {
    ...
    const pdf = await generarPDFComprasPorAlicuota(asientosPdf, periodoLabel(searchParams))
    ...
    return new Response(ab, {
      ...
      headers: {
        ...
        "Content-Disposition": `inline; filename="iva-compras-alicuota-${periodoSlug(searchParams)}.pdf"`,
