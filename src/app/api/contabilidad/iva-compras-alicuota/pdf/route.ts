/**
 * API Route: GET /api/contabilidad/iva-compras-alicuota/pdf
 * Genera el reporte IVA Compras agrupado por Tipo Cbte → Alícuota en HTML imprimible.
 ...
 *
 * Devuelve HTML imprimible con IVA Compras agrupado por tipoCbte → alícuota.
 */
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const asientos = await prisma.asientoIva.findMany({
      ...
      include: {
        facturaProveedor: { select: { tipoCbte: true } },
      },
      orderBy: [{ periodo: "asc" }],
    })
    ...
    const secciones = Array.from(grupoMap.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([tipoCbte, byAlic]) => {
      ...
      return `
        <tr class="header"><td colspan="4">Tipo comprobante: ${tipoCbte}</td></tr>
        ${rows}
...
<head>
  <meta charset="UTF-8">
  <title>IVA Compras por Alícuota — ${periodoLabel}</title>
  <style>
...
<body>
  <h1>Transmagg — IVA Compras por Alícuota</h1>
  ...
  <script>window.onload = function() { window.print() }</script>
    ...
    return serverErrorResponse("GET /api/contabilidad/iva-compras-alicuota/pdf", error)
  }
}
