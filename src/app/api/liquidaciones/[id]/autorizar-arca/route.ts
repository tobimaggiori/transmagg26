 ...
 *
 * Autoriza una liquidación (LP) en ARCA/AFIP. Obtiene CAE, número definitivo
...
import { autorizarLiquidacionArca, ArcaError } from "@/lib/arca"
...
export async function POST(
  ...
  try {
    const result = await autorizarLiquidacionArca(params.id, parsed.data.idempotencyKey)
    ...
    console.error("[POST /api/liquidaciones/[id]/autorizar-arca]", err)
    return NextResponse.json(
