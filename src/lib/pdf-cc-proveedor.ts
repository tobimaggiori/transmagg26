/**
 * Propósito: Generación del PDF de la Cuenta Corriente de un proveedor con pdfkit.
 ...
 *
 * Análogo a `pdf-cc-fletero.ts`. Mantener ambos en paridad cuando se
 * actualice el layout.
 */

import PDFDocument from "pdfkit"

export interface MovimientoCCProveedorPDF {
  fecha: string
  concepto: string
  comprobante: string
  debe: number
...
export interface DatosPDFCCProveedor {
  proveedor: { razonSocial: string; cuit: string }
  movimientos: MovimientoCCProveedorPDF[]
  totalDebe: number
  totalHaber: number
  saldoFinal: number
  desde: Date | null
...
/**
 * generarPDFCCProveedor: DatosPDFCCProveedor -> Promise<Buffer>
 *
 * Dado el proveedor, rango de fechas, movimientos con saldo acumulado y totales,
 * genera un PDF A4 portrait con cabecera (razón social + CUIT + rango) y tabla
 * cronológica de movimientos más totales.
 *
 * Ejemplos:
 * generarPDFCCProveedor({ proveedor, movimientos: [], totalDebe: 0, totalHaber: 0, saldoFinal: 0, desde, hasta })
 * // => Buffer (PDF A4 con mensaje "Sin movimientos en el período")
 * generarPDFCCProveedor({ proveedor, movimientos: [m1, m2], ... })
 * // => Buffer con filas y totales
 */
export async function generarPDFCCProveedor(datos: DatosPDFCCProveedor): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.font("Helvetica-Bold").fontSize(16).fillColor("#000")
      .text(datos.proveedor.razonSocial, 40, cursorY, { width: 515 })
    doc.font("Helvetica").fontSize(10).fillColor("#444")
      .text(`CUIT: ${fmtCuit(datos.proveedor.cuit)}`, 40, doc.y, { width: 515 })
