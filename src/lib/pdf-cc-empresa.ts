/**
 * Propósito: Generación del PDF de la Cuenta Corriente de una empresa con pdfkit.
...
export interface MovimientoCCEmpresaPDF {
  fecha: string
  concepto: string
  comprobante: string
  debe: number
...
export interface DatosPDFCCEmpresa {
  empresa: { razonSocial: string; cuit: string }
  movimientos: MovimientoCCEmpresaPDF[]
  totalDebe: number
  totalHaber: number
  saldoFinal: number
  desde: Date | null
...
/**
 * generarPDFCCEmpresa: DatosPDFCCEmpresa -> Promise<Buffer>
 *
 * Dada la empresa, rango de fechas, movimientos con saldo acumulado y totales,
 * genera un PDF A4 portrait con cabecera (razón social + CUIT + rango) y tabla
 * cronológica de movimientos más totales.
 *
 * Ejemplos:
 * generarPDFCCEmpresa({ empresa, movimientos: [], totalDebe: 0, totalHaber: 0, saldoFinal: 0, desde, hasta })
 * // => Buffer (PDF A4 con mensaje "Sin movimientos en el período")
 *
 * generarPDFCCEmpresa({ empresa, movimientos: [m1, m2], ... })
 * // => Buffer con 2 filas y totales
 */
export async function generarPDFCCEmpresa(datos: DatosPDFCCEmpresa): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    // Cabecera: logo solo arriba-izquierda; los datos de empresa van debajo
    const LOGO_X = 40
    const LOGO_Y = 40
    const LOGO_W = 120
    const LOGO_H = 55
    ...
    doc.font("Helvetica-Bold").fontSize(16).fillColor("#000")
      .text(datos.empresa.razonSocial, 40, cursorY, { width: 515 })
    doc.font("Helvetica").fontSize(10).fillColor("#444")
      .text(`CUIT: ${fmtCuit(datos.empresa.cuit)}`, 40, doc.y, { width: 515 })
