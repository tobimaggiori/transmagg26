...
export async function generarPDFFactura(facturaId: string): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    for (const line of clientLines) {
      line.icon(doc, left + clientBoxPadX, clientY + (clientLineH - clientIconH) / 2)
      ...
      for (const part of line.parts) {
        ...
        doc.text(part.text, textX, clientY + (clientLineH - fontSize) / 2, { lineBreak: false })
        textX += w
      }
      clientY += clientLineH
    }
    ...
    const grupos = agruparViajesPorCupo(fac.viajes.map((v) => ({
    ...
    for (const v of grupos) {
      ...
      if (hasDocBadge) {
        ...
        for (let i = 0; i < partes.length; i++) {
          ...
          doc.text(label, textX, textY, { lineBreak: false })
          textX += doc.widthOfString(label)
          doc.font("Helvetica").fontSize(8.5).fillColor(TEXT)
          doc.text(valor, textX, textY, { lineBreak: false })
