...
End Sub
Public Sub ImprimeCHMacro(VNroMov As Double, Destinatario As String)
Dim NrosCheques As String, Hojas As Integer, MesEmision As String, MesVence As String, MES As Integer
Dim DIA As String, AÑO As String, LINEAY As Double, LineaX As Double, RENGLON1 As String, RENGLON2 As String
Dim cALetra As New clsNum2Let
Set rsDetAdelCHP = db.OpenRecordset("Select * From CHEmitidos Where NroMov = '" & VNroMov & "' AND Adel = 'NO'")
Printer.ScaleMode = 6
Printer.FontSize = 9: Printer.FontBold = True
LineaX = 150: LINEAY = 0
NrosCheques = ""
Do While Not rsDetAdelCHP.EOF
    NrosCheques = NrosCheques & rsDetAdelCHP!NroComp
    rsDetAdelCHP.MoveNext
    NrosCheques = NrosCheques & ", "
Loop
Hojas = InputBox("Cantidad de Cheques en la hoja")
If Hojas >= 1 Then
 MsgBox "Imprime Cheques Nros:" & NrosCheques, vbInformation
rsDetAdelCHP.MoveFirst
Do While Not rsDetAdelCHP.EOF
    If Not Hojas = 0 Then
            MES = Mid(rsDetAdelCHP!FEmision, 4, 2)
            Select Case Val(MES)
            Case 1: MesEmision = "Enero"
            Case 2: MesEmision = "Febrero"
            Case 3: MesEmision = "Marzo"
            Case 4: MesEmision = "Abril"
            Case 5: MesEmision = "Mayo"
            Case 6: MesEmision = "Junio"
            Case 7: MesEmision = "Julio"
            Case 8: MesEmision = "Agosto"
            Case 9: MesEmision = "Septiembre"
            Case 10: MesEmision = "Octubre"
            Case 11: MesEmision = "Noviembre"
            Case 12: MesEmision = "Diciembre"
            End Select
            MES = Mid(rsDetAdelCHP!Fecha, 4, 2)
            Select Case Val(MES)
            Case 1: MesVence = "Enero"
            Case 2: MesVence = "Febrero"
            Case 3: MesVence = "Marzo"
            Case 4: MesVence = "Abril"
            Case 5: MesVence = "Mayo"
            Case 6: MesVence = "Junio"
            Case 7: MesVence = "Julio"
            Case 8: MesVence = "Agosto"
            Case 9: MesVence = "Septiembre"
            Case 10: MesVence = "Octubre"
            Case 11: MesVence = "Noviembre"
            Case 12: MesVence = "Diciembre"
            End Select
            'imprime cheque
            Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
            Printer.Print FormatNumber(rsDetAdelCHP!Haber)
            'fecha emision
            LineaX = 40: LINEAY = LINEAY + 7
            DIA = Mid(rsDetAdelCHP!FEmision, 1, 2)
            AÑO = Mid(rsDetAdelCHP!FEmision, 7, 4)
            Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
            Printer.Print DIA
            LineaX = 47
            Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
            Printer.Print MesEmision
            LineaX = 73
            Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
            Printer.Print AÑO
            'fecha vencimiento
            LineaX = 30: LINEAY = LINEAY + 6
            DIA = Mid(rsDetAdelCHP!Fecha, 1, 2)
            AÑO = Mid(rsDetAdelCHP!Fecha, 7, 4)
            Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
            Printer.Print DIA
            LineaX = 48
            Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
            Printer.Print MesVence
            LineaX = 106
            Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
            Printer.Print AÑO
            'destinatario
            LineaX = 55: LINEAY = LINEAY + 6
            Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
            Printer.Print Destinatario
            'imprime importe en letras
            LineaX = 65: LINEAY = LINEAY + 6
            cALetra.Numero = Val(rsDetAdelCHP!Haber)
            'texto1 = EnLetras(CStr(RSDETADELCHP!Haber))
            Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
            ImpLetras = justificarTexto(cALetra.ALetra, 40, 0)
            cant = Len(ImpLetras)
            i = 0
            RENGLON1 = ""
            RENGLON2 = ""
            For i = i + 1 To cant
                letra = Mid(ImpLetras, i, 1)
                If Not letra = "" Then
                    num = Asc(letra)
                If Not num = 13 Then
                    RENGLON1 = RENGLON1 & Mid(UCase(ImpLetras), i, 1)
                Else
                    cant = cant - 2
                    RENGLON2 = Mid(UCase(ImpLetras), i + 2, cant)
                    Exit For
                End If
                End If
            Next
            If RENGLON2 = "" Then
                Printer.Print RENGLON1 & ".--"
            Else
                Printer.Print RENGLON1
                LINEAY = LINEAY + 6
                LineaX = 40
...
End Sub
Public Sub ImprimeCH(VNroMov As Double, Destinatario As String)
Dim NrosCheques As String, Hojas As Integer, MesEmision As String, MesVence As String, MES As Integer
Dim DIA As String, AÑO As String, LINEAY As Double, LineaX As Double, RENGLON1 As String, RENGLON2 As String
Dim cALetra As New clsNum2Let
Set rsDetAdelCHP = db.OpenRecordset("Select * From CHEmitidos Where NroMov = '" & VNroMov & "' AND Adel = 'NO' AND CodComp = 11")
Printer.ScaleMode = 6
Printer.FontSize = 9: Printer.FontBold = True
LineaX = 150: LINEAY = 2
NrosCheques = ""
...
Do While Not rsDetAdelCHP.EOF
    If Not Hojas = 0 Then
            ...
            Printer.Print AÑO
            'destinatario
            LineaX = 65: LINEAY = LINEAY + 6.5
            Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
            Printer.Print Destinatario
            'imprime importe en letras
            LineaX = 75: LINEAY = LINEAY + 6.5
            cALetra.Numero = Val(rsDetAdelCHP!Haber)
            'texto1 = EnLetras(CStr(RSDETADELCHP!Haber))
            Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
            ImpLetras = justificarTexto(cALetra.ALetra, 40, 0)
...
End If
End Sub
Public Sub ImprimeCHOP(VNroMov As Double, Destinatario As String)
Dim NrosCheques As String, Hojas As Integer, MesEmision As String, MesVence As String, MES As Integer
Dim DIA As String, AÑO As String, LINEAY As Double, LineaX As Double, RENGLON1 As String, RENGLON2 As String
Dim cALetra As New clsNum2Let
Set rsDetAdelCHP = db.OpenRecordset("Select * From CHEmitidos Where NroMov = '" & VNroMov & "' AND CodComp = 11")
Printer.ScaleMode = 6
Printer.FontSize = 9: Printer.FontBold = True
LineaX = 150: LINEAY = 4
NrosCheques = ""
...
End Sub
Public Function ImprimeCHAdel(VNroMov As Double, Destinatario As String)
Dim NrosCheques As String, Hojas As Integer, MesEmision As String, MesVence As String, MES As Integer
Dim DIA As String, AÑO As String, LINEAY As Double, LineaX As Double, RENGLON1 As String, RENGLON2 As String
Dim cALetra As New clsNum2Let
Set rsDetAdelCHP = db.OpenRecordset("Select * From DetAdelCHP Where NroAdel = " & VNroMov & "")
Printer.ScaleMode = 6
Printer.FontSize = 9: Printer.FontBold = True
LineaX = 150: LINEAY = 4
NrosCheques = ""
Do While Not rsDetAdelCHP.EOF
    NrosCheques = NrosCheques & rsDetAdelCHP!NroCH
    rsDetAdelCHP.MoveNext
    NrosCheques = NrosCheques & ", "
Loop
Hojas = InputBox("Cantidad de Cheques en la hoja")
...
Do While Not rsDetAdelCHP.EOF
    If Not Hojas = 0 Then
        ...
        'imprime cheque
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
        Printer.Print FormatNumber(rsDetAdelCHP!Importe)
        'fecha emision
        LineaX = 54: LINEAY = LINEAY + 7
        DIA = Mid(rsDetAdelCHP!FEmision, 1, 2)
        AÑO = Mid(rsDetAdelCHP!FEmision, 7, 4)
        ...
        'fecha vencimiento
        LineaX = 49: LINEAY = LINEAY + 7
        DIA = Mid(rsDetAdelCHP!Vto, 1, 2)
        AÑO = Mid(rsDetAdelCHP!Vto, 7, 4)
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
        Printer.Print DIA
        LineaX = 67
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
        ...
        LineaX = 75: LINEAY = LINEAY + 6.5
        'texto1 = EnLetras(CStr(RSDETADELCHP!Haber))
        cALetra.Numero = Val(rsDetAdelCHP!Importe)
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
        ImpLetras = justificarTexto(cALetra.ALetra, 40, 0)
        cant = Len(ImpLetras)
        i = 0
...
End Function
Public Function justificarTexto(ByVal texto As String, ByVal num As Long, just As Boolean) As String
    Dim tWrap As cWrap
    Set tWrap = New cWrap
    '
    ...
    Set tWrap = Nothing
End Function
Public Function justificarTextoFMiPyme(ByVal texto As String, ByVal num As Long, just As Boolean) As String
    Dim tWrap As cWrap
    Set tWrap = New cWrap
    '
    If num < 1 Or num > Len(texto) Then
    ...
    End If
    '
    justificarTextoFMiPyme = tWrap.PropperJust(texto, num, just)
    '
    Set tWrap = Nothing
End Function
...
With Printer
For i = i + 1 To 2
    Printer.CurrentX = 193: Printer.CurrentY = 18
    Printer.Print NroComp
    Printer.CurrentX = 193: Printer.CurrentY = 23
    ...
    Set rsSituacionIVA = Nothing
    .CurrentX = 223: .CurrentY = 43
    Printer.Print rsEmpresas!Localidad
    .CurrentX = 133: .CurrentY = 50
    ...
    'Printer.Print rsFleteros!Localidad
    Set rsDetFactCta = db.OpenRecordset("Select * From DetFactCta Where NroFact = " & NroComp & " Order By FechaViaje")
    LINEAY = 73
    If rsEncabFactCta!TipoFact = 4 Then
        Do While Not rsDetFactCta.EOF
            .CurrentX = 57: .CurrentY = LINEAY
            Printer.Print rsDetFactCta!mERCADERIA
            .CurrentX = 182: .CurrentY = LINEAY
...
For i = i + 1 To 2
    ...
    Else
        Do While Not rsDetFactCta.EOF
            .CurrentX = 20: .CurrentY = LINEAY
            Printer.Print rsDetFactCta!FechaViaje
            .CurrentX = 41: .CurrentY = LINEAY
            Printer.Print Mid(rsDetFactCta!NroRem, 1, 10)
            .CurrentX = 67: .CurrentY = LINEAY
            Printer.Print Mid(rsDetFactCta!mERCADERIA, 1, 13)
            .CurrentX = 94: .CurrentY = LINEAY
            Printer.Print Mid(rsDetFactCta!pROCEDENCIA, 1, 13)
            .CurrentX = 121: .CurrentY = LINEAY
            Printer.Print Mid(rsDetFactCta!dESTINO, 1, 50)
            .CurrentX = 151: .CurrentY = LINEAY
            Printer.Print FormatNumber(rsDetFactCta!kilos)
            .CurrentX = 171: .CurrentY = LINEAY
            Printer.Print FormatNumber(rsDetFactCta!tarifa)
            .CurrentX = 197: .CurrentY = LINEAY
            Printer.Print FormatNumber(rsDetFactCta!STotal)
            Set rsFleteros = db.OpenRecordset("Select * From Fleteros Where CodFlet = " & rsDetFactCta!codflet & "")
            .CurrentX = 218: .CurrentY = LINEAY
            Desc = Mid(rsFleteros!DescFlet, 1, 16)
            Printer.Print Desc
            .CurrentX = 250: Printer.CurrentY = LINEAY
            Printer.Print rsFleteros!cuit
            .CurrentX = 276: .CurrentY = LINEAY
            Printer.Print "R.I."
            LINEAY = LINEAY + 7
            rsDetFactCta.MoveNext
        Loop
...
End With
End Sub
Public Sub Imprime_NCA(NroComp As Long)
...
For i = i + 1 To 2
    ...
    If rsEncabFact!TipoFact = 2 Then
        ...
        Printer.Print rsDetFact!ConceptoNC
...
End Sub
Public Sub ConsultaFactVieja(NroComp As Long)
Set rsEncabFact = db.OpenRecordset("Select * From EncabFact Where NroFact = " & NroComp & "")

    With TrsEncabFact
        .AddNew
        .Fields("NroFact") = rsEncabFact!NroFact
        .Fields("Fecha") = rsEncabFact!Fecha
        .Fields("Codigo") = rsEncabFact!Codigo
        Set rsEmpresas = db.OpenRecordset("Select * From Empresas Where CodEmpresas = " & rsEncabFact!Codigo & "")
        ...
        .Fields("TNeto") = rsEncabFact!TNeto
        .Fields("TIVa") = rsEncabFact!TIVA
        .Fields("TGRAL") = rsEncabFact!TGral
        largo = Len(rsEncabFact!NroFact)
        Select Case largo
        Case 1: NRO = "0000000" & rsEncabFact!NroFact
        Case 2: NRO = "000000" & rsEncabFact!NroFact
        Case 3: NRO = "00000" & rsEncabFact!NroFact
        Case 4: NRO = "0000" & rsEncabFact!NroFact
        Case 5: NRO = "000" & rsEncabFact!NroFact
        Case 6: NRO = "00" & rsEncabFact!NroFact
        Case 7: NRO = "0" & rsEncabFact!NroFact
        Case 8: NRO = rsEncabFact!NroFact
        End Select
        .Fields("NroFE") = NRO
...
End Sub
Public Sub Imprime_Fact(NroComp As Long)
Set rsEncabFact = db.OpenRecordset("Select * From EncabFact Where NroFact = " & NroComp & "")
Printer.PaperSize = 9
Printer.ScaleMode = 6
i = 0
...
End With
End Sub
Public Function VistaOP(NroOP As Long)
Set rsEncabOP = db.OpenRecordset("Select * From EncabOP Where NroOP = " & NroOP & "")
Set TrsEncabOP = dbTemp.OpenRecordset("EncabOP")
'TrsEncabOP.MoveFirst
Do While Not TrsEncabOP.EOF
    TrsEncabOP.Delete
    TrsEncabOP.MoveNext
Loop
With TrsEncabOP
        .AddNew
        .Fields("NroOP") = rsEncabOP!NroOP
        .Fields("Fecha") = rsEncabOP!Fecha
        .Fields("CodProv") = rsEncabOP!codprov
        .Fields("TotalOP") = rsEncabOP!totalop
        '.Fields("TNComis") = TNComis
        '.Fields("IVAComis") = TIVAComis
        '.Fields("TComis") = TComis
        .Fields("TAdelantos") = rsEncabOP!Tadelantos
        .Fields("TGasOil") = rsEncabOP!TGasOil
        .Fields("TFalt") = rsEncabOP!TFalt
        .Fields("TEfvo") = rsEncabOP!TEfvo
        .Fields("TCHPropio") = rsEncabOP!Tchpropio
        .Fields("TCHTerceros") = rsEncabOP!TCHTerceros
        .Update
    End With
    Set TrsEncabOP = Nothing
    VNROOP = NroOP
    Dim frmRep As New InfOrdenPago
    frmRep.Show vbModal
End Function

Public Function ImprimeOP(NroOP As Long)
Dim Derecha As Boolean
Dim TotFactAplic As Double
i = 0
Printer.PaperSize = 9
Printer.ScaleMode = 6
Printer.Font = Arial
...
'For I = I + 1 To 2
    ...
    End Select
    Printer.Print "Orden Nro:" & "  0001" & "-" & VNro
    ...
    Do While Not rsAplicOP.EOF
        If LINEAY < 270 Then
            ...
            Else
                If Not rsAplicOP!impaplic = 0 Then
                ...
                End If
            End If
            TotFactAplic = TotFactAplic + rsAplicOP!impaplic
            LINEAY = LINEAY + 5
            rsAplicOP.MoveNext
        Else
    ...
    Printer.CurrentX = LineaX + 90: Printer.CurrentY = LINEAY
    Printer.Print FormatCurrency(TotFactAplic)
    Printer.FontBold = False
    LINEAY = LINEAY + 5
    ...
    Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
    Printer.FontBold = True: Printer.FontItalic = True: Printer.FontSize = 14
    Printer.Print "Detalle Orden de Pago"
    Printer.FontBold = False: Printer.FontItalic = False: Printer.FontSize = 10
    LINEAY = LINEAY + 5
    If LINEAY < 275 Then
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY: Printer.FontUnderline = True
        Printer.Print "Efectivo:"
        Printer.CurrentX = LineaX + 90: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
    ...
    LINEAY = LINEAY + 5
    Set rsDetOPAdel = db.OpenRecordset("sELECT * from DetOpAdel Where NroOP = " & rsEncabOP!NroOP & "")
    If LINEAY < 275 Then
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY: Printer.FontUnderline = True
        Printer.Print "Adelantos:"
        LINEAY = LINEAY + 5
        Printer.CurrentX = LineaX + 5: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
        Printer.Print "Efectivo"
        Printer.CurrentX = LineaX + 25: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
        Printer.Print "GasOil"
        Printer.CurrentX = LineaX + 45: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
        Printer.Print "Faltante"
        Printer.CurrentX = LineaX + 65: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
        Printer.Print "Remito"
        Do While Not rsDetOPAdel.EOF
            LINEAY = LINEAY + 5
            Printer.CurrentX = LineaX + 5: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
            Printer.Print FormatCurrency(rsDetOPAdel!Efvo)
            Printer.CurrentX = LineaX + 25: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
            Printer.Print FormatCurrency(rsDetOPAdel!gASOIL)
            Printer.CurrentX = LineaX + 45: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
            Printer.Print FormatCurrency(rsDetOPAdel!Faltante)
            Printer.CurrentX = LineaX + 65: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
            Printer.Print rsDetOPAdel!NroRemito
            rsDetOPAdel.MoveNext
        Loop
        LINEAY = LINEAY + 5
        
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
        Printer.Print "Total Adelantos"
        Printer.CurrentX = LineaX + 90: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
        Printer.Print FormatCurrency(rsEncabOP!Tadelantos + rsEncabOP!TGasOil + rsEncabOP!TFalt)
    Else
        If Derecha = False Then
            LINEAY = 85
            LineaX = 110
    ...
    Else
        ...
        Else
            ...
            Printer.Print "Cheques de Terceros"
            Printer.FontUnderline = False
            Set rsEncabOP = db.OpenRecordset("Select * From EncabOP Where NroOP = " & rsEncabOP!NroOP & "")
            LINEAY = LINEAY + 5
        End If
    End If
    ...
    Else
        ...
        Else
            ...
            Printer.Print "Cheques Propios"
            Printer.FontUnderline = False
            Set rsDetAdelCHP = db.OpenRecordset("Select * From DetAdelCHP Where NroOP = " & rsEncabOP!NroOP & "")
            LINEAY = LINEAY + 5
        End If
    End If
    ...
    If TotFactAplic < rsEncabOP!totalop Then
        acuenta = rsEncabOP!totalop - totalfactaplic
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY + 5
        Printer.Print "A CUENTA"
        Printer.CurrentX = LineaX + 90: Printer.CurrentY = LINEAY + 5
        Printer.Print FormatCurrency(acuenta)
    End If
    Printer.FontBold = False
    Printer.NewPage
'Next
...
End Function
Public Function ImprimeAdel(NroAdel As Long)
Dim Derecha As Boolean
Dim TotFactAplic As Double
i = 0
Printer.PaperSize = 9
...
'For I = I + 1 To 2
    ...
    Printer.Line (10, 270)-(200, 270)
    'imprime encabezado
    Set rsEncabAdel = db.OpenRecordset("Select * From EncabAdel Where NroAdel = " & NroAdel & "")
    Printer.CurrentX = 15: Printer.CurrentY = 10
    Printer.FontSize = 14: Printer.FontBold = True
    Printer.Print "TRANSPORTE TRANS-MAGG"
    Printer.FontSize = 10: Printer.FontBold = False
    Printer.CurrentX = 115: Printer.CurrentY = 10
    Printer.FontBold = True
    Printer.Print "Adelanto"
    Printer.FontBold = False
    Printer.CurrentX = 115: Printer.CurrentY = 15
    Tamaño = Len(rsEncabAdel!NroAdel)
    Select Case Tamaño
        Case 1: VNro = "0000000" & rsEncabAdel!NroAdel
        Case 2: VNro = "000000" & rsEncabAdel!NroAdel
        Case 3: VNro = "00000" & rsEncabAdel!NroAdel
        Case 4: VNro = "0000" & rsEncabAdel!NroAdel
        Case 5: VNro = "000" & rsEncabAdel!NroAdel
        Case 6: VNro = "00" & rsEncabAdel!NroAdel
        Case 7: VNro = "0" & rsEncabAdel!NroAdel
        Case 8: VNro = rsEncabAdel!NroAdel
    End Select
    Printer.Print "Adelanto Nro:" & "  0001" & "-" & VNro
    Printer.CurrentX = 115: Printer.CurrentY = 20
    Printer.Print "Fecha:" & "  " & rsEncabAdel!Fecha
    Printer.CurrentX = 15: Printer.CurrentY = 45
    Set rsFleteros = db.OpenRecordset("Select * From Fleteros Where CodFlet = " & rsEncabAdel!codprov & "")
    Printer.Print "Sres: " & "   " & rsEncabAdel!codprov & "    " & rsFleteros!DescFlet
    Printer.CurrentX = 15: Printer.CurrentY = 55
    Printer.Print "Domicilio: " & rsFleteros!Direccion
    Printer.CurrentX = 115: Printer.CurrentY = 55
    Printer.Print "Localidad: " & rsFleteros!Localidad
    ...
    Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
    Printer.FontBold = True: Printer.FontItalic = True: Printer.FontSize = 14
    Printer.Print "Detalle Adelantos"
    Printer.FontBold = False: Printer.FontItalic = False: Printer.FontSize = 10
    LINEAY = LINEAY + 5
    If LINEAY < 275 Then
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY: Printer.FontUnderline = True
        Printer.Print "Efectivo:"
        Printer.CurrentX = LineaX + 60: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
        Printer.Print FormatCurrency(rsEncabAdel!TEfvo)
    Else
        If Derecha = False Then
            LINEAY = 85
            LineaX = 110
    ...
    End If
    LINEAY = LINEAY + 5
    Set rsDetAdelCHT = db.OpenRecordset("Select * From DetAdelCHT Where NroAdel = " & rsEncabAdel!NroAdel & "")
    If LINEAY < 275 Then
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY: Printer.FontUnderline = True
        Printer.Print "Cheques de Terceros"
        Printer.FontUnderline = False
    ...
    Do While Not rsDetAdelCHT.EOF
        If LINEAY < 275 Then
            ...
            Set rsBcos = Nothing
            Printer.CurrentX = LineaX + 30: Printer.CurrentY = LINEAY
            Printer.Print rsDetAdelCHT!NroCH
            Printer.CurrentX = LineaX + 60: Printer.CurrentY = LINEAY
            Printer.Print FormatCurrency(rsDetAdelCHT!Importe)
            LINEAY = LINEAY + 5
    ...
    Printer.Print "Total Cheques de Terceros:"
    Printer.CurrentX = LineaX + 60: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
    Printer.Print FormatCurrency(rsEncabAdel!TCHTerceros)
    Set rsDetAdelCHT = Nothing
    LINEAY = LINEAY + 5
    If LINEAY < 275 Then
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY: Printer.FontUnderline = True
        Printer.Print "Cheques Propios"
        Printer.FontUnderline = False
        Set rsDetAdelCHP = db.OpenRecordset("Select * From DetAdelCHP Where NroAdel = " & rsEncabAdel!NroAdel & "")
        LINEAY = LINEAY + 5
    Else
        If Derecha = False Then
            LINEAY = 85
    ...
    Do While Not rsDetAdelCHP.EOF
        If LINEAY < 275 Then
            ...
            Set rsCtaBcoPropias = Nothing
            Printer.CurrentX = LineaX + 30: Printer.CurrentY = LINEAY
            Printer.Print rsDetAdelCHP!NroCH
            Printer.CurrentX = LineaX + 60: Printer.CurrentY = LINEAY
            Printer.Print FormatCurrency(rsDetAdelCHP!Importe)
            LINEAY = LINEAY + 5
            rsDetAdelCHP.MoveNext
        Else
            If Derecha = False Then
        ...
        End If
    Loop
    Set rsDetAdelCHP = Nothing
    LINEAY = LINEAY + 5
    Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY: Printer.FontUnderline = True
    Printer.Print "Total Cheques Propios"
    Printer.CurrentX = LineaX + 60: Printer.CurrentY = LINEAY: Printer.FontUnderline = False
    Printer.Print FormatCurrency(rsEncabAdel!Tchpropio)
    Printer.FontBold = True
    LINEAY = LINEAY + 5
    'imprime detalle del Adelanto
    Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
    Printer.Print "TOTAL DETALLE"
    Printer.CurrentX = LineaX + 60: Printer.CurrentY = LINEAY
    Printer.Print FormatCurrency(rsEncabAdel!TotalAdel)
    'If TotFactAplic < rsEncabAdel!TotalAdel Then
    '    acuenta = rsEncabAdel!TotalAdel - totalfactaplic
    '    Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY + 5
    '    Printer.Print "A CUENTA"
    '    Printer.CurrentX = LineaX + 60: Printer.CurrentY = LINEAY + 5
    '    Printer.Print FormatCurrency(acuenta)
    'End If
    Printer.FontBold = False
    Printer.NewPage
    ...
    Do While Not rsAplicRec.EOF
        If LINEAY > 260 Then
            ...
            Else
                ...
                Printer.Print "CUIT: " & rsEmpresas!cuit
                Set rsEmpresas = Nothing
                LINEAY = 85: LineaX = 80: Derecha = False
            End If
        End If
        Printer.CurrentX = LineaX: Printer.CurrentY = LINEAY
...
Public Function Exportar_excell1()
   ...
   ' -- Abrir la base
   Set TrsChoferes = dbTemp.OpenRecordset("select * from InfChoferes Order by CodFlet")
    ...
    iCol = TrsChoferes.Fields.Count
    For iCol = 1 To TrsChoferes.Fields.Count
        Hoja.Cells(1, iCol).Value = TrsChoferes.Fields(iCol - 1).Name
    Next
      
    If Val(Mid(Excel.Version, 1, InStr(1, Excel.Version, ".") - 1)) > 8 Then
        Hoja.Cells(2, 1).CopyFromRecordset TrsChoferes
    Else
  
        arrData = TrsChoferes.GetRows
  
        iRec = UBound(arrData, 2) + 1
          
        For iCol = 0 To TrsChoferes.Fields.Count - 1
        ...
        Hoja.Cells(2, 1).Resize(iRec, TrsChoferes.Fields.Count).Value = GetData(arrData)
    End If
    ...
    Set TrsChoferes = Nothing

    ' -- guardar el libro
    Libro.SaveAs App.Path + "\Excell TransMagg\listado_de_choferes.xls"
    Libro.Close
