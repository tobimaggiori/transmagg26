...
Begin VB.Form IVACompras 
   ...
   Begin KewlButtonz.KewlButtons Exportar 
      ...
      VALUE           =   0   'False
   End
   Begin KewlButtonz.KewlButtons GastosConceptos 
      Height          =   495
      Left            =   480
      TabIndex        =   9
      Top             =   3360
...
Else
    ...
    Do While Not rsEncabFactProv.EOF
        ...
        Do While Not rsDetFactProv.EOF
            If Not rsDetFactProv!CodConcepto = 14 And Not rsDetFactProv!CodConcepto = 16 And Not rsDetFactProv!CodConcepto = 17 And Not rsDetFactProv!CodConcepto = 18 Then
                ...
                If rsEncabFactProv!CodComp = 1 Or rsEncabFactProv!CodComp = 2 Then
                    If Not IsNull(rsDetFactProv!Importe) Then
        ...
        If Not FormatNumber(VTotalNeto) = FormatNumber(rsEncabFactProv!TotalNeto) Or Not FormatNumber(rsEncabFactProv!IVA) = FormatNumber(viva2, 2) Then
            ...
            vdifiva = FormatNumber(rsEncabFactProv!IVA) - FormatNumber(viva2, 2)
            MsgBox "Error en factura Nro: " & rsEncabFactProv!NroFact & "Diferencia:" & FormatNumber(VDif) & "IVA" & FormatNumber(vdifiva)
            .AddNew
            .Fields("CodComp") = rsEncabFactProv!CodComp
            Set rsComprobantes = db.OpenRecordset("Select * From Comprobantes Where CodComp = " & rsEncabFactProv!CodComp & "")
            .Fields("DescComp") = rsComprobantes!DescComp
    ...
    Do While Not rsEncabFactProv.EOF
           ...
           .AddNew
            ...
            .Fields("DescComp") = rsComprobantes!DescComp
            ivaviaje = rsEncabFactProv!netoviajes * 21 / 100
            If FormatNumber(ivaviaje) = FormatNumber(rsEncabFactProv!ivaviaje) Then
             Else
                VDif = FormatNumber(ivaviaje) - FormatNumber(rsEncabFactProv!ivaviaje)
                MsgBox "Dif en liquido producto Nro: " & rsEncabFactProv!NroComp & "  Diferencia" & FormatNumber(VDif)
...
Do While Not rsEncabFactProv.EOF
   ...
   'importe total de la operacion
    ...
    'PAGOS A CUENTAS
    'PERC IVA
     VPagoACta = rsEncabFactProv!perciva + rsEncabFactProv!OtrosImp
    vimporte = ""
    If Not rsEncabFactProv!perciva = "" Then
        VTamaño = Len(FormatNumber(VPagoACta))
        i = 0
        For i = i + 1 To VTamaño + 1
            DIGITO = Mid(FormatNumber(VPagoACta), i, 1)
            If Not DIGITO = "." And Not DIGITO = "," Then
                vimporte = vimporte & DIGITO
            End If
        Next
    ...
    VLinea = VLinea & VImpTotal
    'PERC GANANCIA
    VPagoACta = rsEncabFactProv!retganancia
...
Private Sub GastosConceptos_Click()
...
End If
    ...
    With TrsConsultas
        ...
        .Update
    End With
Set TrsGastosConceptos = dbTemp.OpenRecordset("GastosConceptos")
Do While Not TrsGastosConceptos.EOF
        TrsGastosConceptos.Delete
        TrsGastosConceptos.MoveNext
Loop
Set rsEncabFactProv = db.OpenRecordset("SELECT * FROM encabfactProv WHERE Fecha BETWEEN # " + Format(FDESDE, "mm/dd/yyyy") + " # AND # " + Format(FHASTA, "mm/dd/yyyy") + " # AND LIVA = 'SI' ORDER BY Fecha")
With TrsGastosConceptos
Do While Not rsEncabFactProv.EOF
    Set rsDetFactProv = db.OpenRecordset("Select * From DetFactProv Where Id = " & rsEncabFactProv!ID & " and nrofact = " & rsEncabFactProv!NroFact & "")
    Do While Not rsDetFactProv.EOF
        'If Not rsDetFactProv!CodConcepto = 14 And Not rsDetFactProv!CodConcepto = 16 And Not rsDetFactProv!CodConcepto = 17 And Not rsDetFactProv!CodConcepto = 18 Then
            .AddNew
            Tamaño = Len(rsEncabFactProv!PtoVta)
            Select Case Tamaño
                Case 1: VNro = "000" & rsEncabFactProv!PtoVta
                Case 2: VNro = "00" & rsEncabFactProv!PtoVta
                Case 3: VNro = "0" & rsEncabFactProv!PtoVta
                Case 4: VNro = rsEncabFactProv!PtoVta
            End Select
            .Fields("PtoVta") = VNro
            Tamaño = Len(rsEncabFactProv!NroFact)
            Select Case Tamaño
            ...
            End Select
            .Fields("NroComp") = VNro
            .Fields("FechaComp") = rsEncabFactProv!Fecha
            Set rsFleteros = db.OpenRecordset("Select * From Fleteros Where CodFlet = " & rsEncabFactProv!codprov & "")
            .Fields("Proveedor") = rsFleteros!DescFlet
            If rsEncabFactProv!CodComp = 2 Then
                .Fields("CodConcepto") = rsDetFactProv!CodConcepto
                .Fields("Importe") = rsDetFactProv!Importe * -1
            Else
                .Fields("CodConcepto") = rsDetFactProv!CodConcepto
                .Fields("Importe") = rsDetFactProv!Importe
            End If
            .Update
        'End If
        rsDetFactProv.MoveNext
...
Do While Not rsEncabFactProv.EOF
   ...
   Tamaño = Len(rsEncabFactProv!PtoVta)
    ...
    .Fields("Importe") = rsEncabFactProv!netoviajes
    .Update
    rsEncabFactProv.MoveNext
Loop
...
End With
'////////////
    Dim frmRep As New infDetalleGastos
    frmRep.Show vbModal
