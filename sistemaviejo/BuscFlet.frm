...
Set rsFleteros = db.OpenRecordset("SELECT * FROM Fleteros WHERE TipoProv = 0")
...
If Viene = "LiqProd" Then
    ...
    If Not IsNull(VCodflet) Then
        Set rsFleteros = db.OpenRecordset("Select * from Fleteros Where CodFlet = " & VCodflet & "", 2)
            If Not rsFleteros.EOF And Not rsFleteros.BOF Then
                LiqProducto.Text1(0) = rsFleteros!CodFlet
                LiqProducto.Text1(1) = rsFleteros!DescFlet
                LiqProducto.Text1(2) = FormatNumber(rsFleteros!Comision)
...
If Viene = "FactProv" Then
    Set Lista = ListFlet.ListItems.Item(ListFlet.SelectedItem.Index)
    FactProv.Text1(2) = Lista.Tag
    FactProv.Text1(3) = Lista.SubItems(1)
